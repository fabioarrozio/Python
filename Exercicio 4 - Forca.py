...
def main():
    ...
    print('F O R C A')
    letrasErradas = '' #string de letras erradas vazia
    letrasAcertadas = '' #string de letras acertadas vazia
    palavraSecreta = geraPalavraAleatoria().upper()
    jogando = True
 
    while jogando: #enquanto jogando for true
        imprimeJogo(letrasErradas, letrasAcertadas, palavraSecreta) #chama a função imprime jogo
 
        palpite = recebePalpite(letrasErradas + letrasAcertadas) # palpite vai receber função com as letras erradas e a letras certas
 
        if palpite in palavraSecreta: #se o palpite for uma letra que ta na palavra certa
            letrasAcertadas += palpite # minhas letras certas terão que adicionar 1 + o palpite
 
            if VerificaSeGanhou(palavraSecreta, letrasAcertadas): # aqui nós verificamos se o jogo foi vencido
                print("Parabens! A palavra secreta é "+palavraSecreta+'! Você ganhou!!')
                jogando = False
        else:
            ...
            if len(letrasErradas) == len(FORCAIMG) - 1: # verifica se o numero de letras erradas e igual o numero de imagens da forca
                imprimeJogo(letrasErradas, letrasAcertadas, palavraSecreta) # se sim vai ser impresso o jogo
...
def VerificaSeGanhou(palavraSecreta, letrasAcertadas):
    ...
    ganhou = True
    for letra in palavraSecreta: #percorre cada uma das letras da palavra secreta
        if letra not in letrasAcertadas: # se a letra nao esta nas letras acertadas
            ganhou = False #ganhou passa a ser falso
            break # o for loop é parado
